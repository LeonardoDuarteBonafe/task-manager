...
export function OccurrenceItem({ occurrence, onComplete, onIgnore, onOpen, onViewTask, loadingActionId }: OccurrenceItemProps) {
  ...
  return (
    <Card className="overflow-hidden p-0">
      <div className="flex flex-col gap-0 md:flex-row">
        <button className="min-w-0 flex-1 cursor-pointer px-5 py-4 text-left" onClick={() => onOpen?.(occurrence.id)} type="button">
        ...
        <div className="flex w-full shrink-0 flex-col justify-between gap-3 border-t border-slate-200 bg-slate-50/70 px-5 py-4 md:w-[20%] md:min-w-[140px] md:max-w-[300px] md:border-l md:border-t-0 dark:border-slate-800 dark:bg-slate-950/40">
          ...
          <div className="flex flex-wrap gap-2 md:justify-end">
            ...
            <Button onClick={() => onViewTask?.(occurrence.task.taskCode)} type="button" variant="secondary">
              Ver tarefa
            </Button>
          </div>
        </div>
